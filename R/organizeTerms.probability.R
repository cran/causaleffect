...
function(obj) {
  if (obj$recursive) {
    ...
    if (length(children.rec) > 0) for(i in 1:length(children.rec)) obj$children[[i]] <- organizeTerms(children.rec[[i]])    
    ...
    if (obj$fraction) obj$divisor <- organizeTerms(obj$divisor)
  }
  return(obj)
}
